            ...
            <div className='border-t border-gray-100 pt-2 mt-2'>
              ...
              {selectedServices.map((service, index) => (
                ...
                >
                  ...
                  <span>{formatPrice((service.price || 0) * service.quantity)} сом</span>
                </div>
              ))}
              <div className='flex justify-between items-center text-sm font-medium border-t border-gray-100 pt-1 mt-1'>
                ...
                <span>{formatPrice(servicesPrice)} сом</span>
              </div>
            </div>
          )}

          {/* Блок скидки контрагента для ролей не-партнеров */}
          {userRole !== 'partner' && sale && sale > 0 && (
            <div className='flex justify-between items-center text-green-600 border-t border-gray-100 pt-2 mt-2'>
              <span className='text-sm font-medium'>
                Скидка контрагента ({Math.round(sale * 100)}%):
              </span>
              <span className='font-medium'>
                -{formatPrice(Math.max(0, fullPriceValue - totalPriceValue))}
              </span>
            </div>
          )}
                    ...
                    <div className='space-y-1 text-sm'>
                      <div className='flex justify-between'>
                        ...
                        <span className='font-medium line-through'>
                          {formatPrice(calculatedPrice)} KGS
                        </span>
                      </div>
                      ...
                      <div className='flex justify-between'>
                        ...
                        <span
                          className={`font-medium ${isPriceHigher ? 'text-green-600' : 'text-red-600'}`}
                        >
                          {isPriceHigher ? '+' : '-'}
