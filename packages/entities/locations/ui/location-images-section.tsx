...
import type { LocationImageDTO } from '../interface/LocationDTO';
import type { LocationCreateFormData } from '../schemas/locationCreateSchema';

export type ExistingImageItem = { kind: 'existing'; id: string; path: string };
export type PendingImageItem = { kind: 'pending'; file: File; previewUrl: string; error?: string };
export type ImageItem = ExistingImageItem | PendingImageItem;
...
export function LocationImagesSection({
  ...
  const { formState: { errors } } = useFormContext<LocationCreateFormData>();
  ...
  const getSrc = (item: ImageItem) =>
    item.kind === 'existing' ? getUploadUrl(item.path) : item.previewUrl;

  return (
    <div className="space-y-4">
      <Label className="text-sm font-medium">Картинки локации</Label>
      ...
      {(errors as Record<string, { message?: string }>).images?.message && (
        <p className="text-sm text-red-600">{(errors as Record<string, { message?: string }>).images?.message}</p>
      )}
    </div>
  );
}
